...
각 항목은 (A)/(B)/(C) 형태의 명확한 선택지를 제시한다. 사용자 응답을 받고 다음 항목으로 넘어간다.
...
- **`수정`** → 수정 가능한 항목 목록을 번호로 출력하고 선택하게 한다:
  ...
  번호를 입력하세요:
  ```
  선택된 항목의 타입에 따라 Step 5-A / 5-B / 5-C (add 스킬 방식)로 재수집한 뒤 Step 8로 돌아온다.
- **`y`** → Step 9로 진행.
...
Read 툴로 파일을 읽고, 각 타입 배열에 항목을 순서대로 추가한다 (Edit 툴 사용).

- Object Types → `object_types` 배열 끝에 모두 추가
- Link Types → `link_types` 배열 끝에 모두 추가
- Action Types → `action_types` 배열 끝에 모두 추가
...
각 파일을 Read로 읽고 배열 끝에 해당 타입 항목들을 추가한다 (Edit 툴). 빈 배열 처리는 9-B와 동일.
...
추가된 타입이 없는 경우 (모두 기존 타입과 병합/제거된 경우):
```
