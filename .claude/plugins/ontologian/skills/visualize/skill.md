...
- **1개 도메인일 때 선택 메뉴 출력** → 도메인이 1개면 사용자 입력 없이 자동 선택한다.
- **빈 배열 처리 누락** → `object_types`, `link_types`, `action_types` 키가 파일에 없을 수 있다. 없으면 빈 배열로 처리하고 `(없음)` 을 출력한다.
- **플래그 조건 혼동** → `primary: true`/`computed: true`인 경우에만 플래그를 표기한다. `false`이거나 필드가 없으면 생략한다.
- **박스 너비 불일치** → 박스의 상단(`┌─...─┐`), 구분선(`├─...─┤`), 하단(`└─...─┘`) 길이는 항상 동일해야 한다.
- **`all` 선택 시 구분선 누락** → 도메인 사이에 반드시 `════════════════════` 구분선을 출력한다.
- **카디널리티 변환 오류** → YAML 원본값(`one_to_many` 등)을 그대로 출력하지 않는다. 반드시 표기 변환 테이블을 적용한다.
